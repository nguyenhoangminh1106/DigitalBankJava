...
public class Bank {
    ...
    public void addAccount(String customerId, Account newAccount) {
        if (isValidAccount(customerId, newAccount)) { // Neu tai khoan hop le
    ...
    public  boolean isValidAccount (String customerId, Account newAccount) {
        if (isCustomerExisted(customerId)) {// Neu ID hop le
            for (Customer customer : this.getCustomers()) {// Duyet qua tung khach hang trong ngan hang
                if (!customer.isValidAccount(newAccount)) {// Neu tai khoan moi hop le voi tung khach hang duoc duyet
                    return false;
                }
            }
            return true;
...
//////////////// Method tuong tac voi nguoi dung ////////////////
    ...
    public static void hienThiGioiThieu() {
        createBorder(); // Tao vien
        System.out.println("| NGAN HANG SO | " + Asm02.AUTHOR +"@"+ Asm02.VERSION + " |");
        createBorder();
    }
    ...
    } // Tao vien giao dien cho ung dung
    ...
    public void themTaiKhoanKhachHang() {
        ...
        do {
            try {
                ...
                if (!account.isValidAccountNumber(stk)) {
                    ...
                    if (this.isValidAccount(soCCCD, account)) {
        ...
        } while(!account.isValidAccountNumber(stk) || !this.isValidAccount(soCCCD, account));
    }
